...
exports.createVote = async (req, res) => {
    ...
    const { electionId, studentId, iDs } = req.body;

    let transaction;
    try {
        const election = await ElectionModel.findByPk(electionId);
        if (!election) {
            return res.status(404).json({ success: false, message: 'Election non trouvée' });
        }
        ...
        if (existingVote) {
            return res.status(409).json({ success: false, message: 'Cet étudiant a déjà voté pour cette année' });
        }
